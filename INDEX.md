...
**Compiled 2026-08-17. Updated 2026-09-10.** Companion to [`FINDINGS.md`](FINDINGS.md), the flat registry of **314** findings (**F-304→F-314 added 2026-09-10, the catcher and framing as a contaminant of command measurement — eight of eleven are pure derivation; see §7**; F-294→F-303 added 2026-09-09, sequencing arithmetic plus a PARTIAL RETRACTION of this program's own fabrication verdict — see §7; F-285→F-293 added 2026-09-08, arithmetic plus one citation correction; F-277→F-284 added 2026-09-07, all source-independent arithmetic; F-268→F-276 added 2026-09-06; F-260→F-266 added 2026-09-05; F-251→F-259 added 2026-09-04; **F-255, F-256 and F-258 are UNVERIFIED leads in quarantine — see §7**).

> ### 🚨 EGRESS HAS NOW FAILED **SEVEN TIMES IN A ROW** (2026-09-04, -05, -06, -07, -08, -09, -10)
> **Zero primary sources opened across seven consecutive cycles.** `WebSearch` works; `WebFetch` and raw `curl` do not. **The diagnosis is now UNAMBIGUOUS and no future cycle needs to re-litigate it.** On 2026-09-10 three channels agreed at once: `curl` returned `CONNECT tunnel failed, response 403` for `en.wikipedia.org` **used as a control** alongside every journal host; `WebFetch` returned a **new explicit error type, `EGRESS_BLOCKED`**, naming the domain; and the agent proxy's own `/__agentproxy/status` endpoint **now logs the denials itself** — `kind: "connect_rejected"`, `detail: "gateway answered 403 to CONNECT (policy denial or upstream failure)"` — **where on 2026-09-06 it reported ZERO relay failures.** That last change closes the final ambiguity about where the block sits. **This is an empty network allowlist in the remote execution environment. It is not an outage, not a paywall, and not a proxy fault — IT REQUIRES A HUMAN TO CHANGE** (F-277, F-285, **F-304**).
> **The standing brief's WORKING-domain list has been wrong SEVEN times and should be DELETED from the prompt, not amended** — replaced with *"probe first; if blocked, close a gap with arithmetic."*
> **PROBE EGRESS BEFORE CHOOSING A TOPIC.** This instruction was added after cycle 4, has been followed on 2026-09-08, -09 and -10, cost ninety seconds each time and determined the whole shape of the cycle. **KEEP IT.** If egress is down, pick a gap that arithmetic can close (F-266, F-278, F-286, F-295, **F-305/F-307**).
> **⚠️ THE UNREAD VERIFICATION QUEUE NOW STANDS AT 27 ITEMS. Seven consecutive arithmetic-only cycles have produced ~60 findings, essentially all internally consistent and NONE checked against data.** That is the condition F-259 warns about, and the corpus is now deep enough into it that **the next cycle with working egress should spend itself entirely on the queue rather than opening a new topic.** Queue head, unchanged: **arXiv `2601.11904` (pitch-pattern motifs)** — it would test F-299 directly AND resolve the F-284/F-303 retraction in one page — then the Brill/Deshpande/Wyner TTOP paper (F-290, F-292), then the Kovash & Levitt / *Sports Economics Review* minimax pair (F-302). **+4 added 2026-09-10:** PMID 42423608 (JSCR longitudinal CMJ, **n = 8**), PMID 42646876 (OpenBiomechanics secondary analysis, a candidate **public in-band dataset**), the Sports Info Solutions ABS/framing article (F-309, F-312), and PMID 42253247 itself — **the corpus's most population-matched paper has still never been read on a page** (F-010, F-311).
> **⚠️ THIS INSTRUCTION IS NOW THREE CYCLES OLD AND HAS NEVER ONCE BEEN EXECUTABLE.**
...
| **`library/open-disputes.md`** | ~530 lines / 66 KB | **2026-09-10** | **Twenty-three** unresolved arguments among the anatomist, the biomechanist and the coach, each with both sides, the strongest evidence, and what would settle it. Plus the standing methodological disputes | "Is this settled?" "What would I have to run to find out?" "Where do the three specialists actually disagree?" |
...
| **`library/catcher-framing-command.md`** | **NEW 2026-09-10** | 2026-09-10 | **The catcher, and why a called-strike rate is not a measurement of the pitcher.** The shadow annulus is larger than the strike zone; the framing-to-inches conversion (~14 pp/in) that shrinks the whole framing industry to a fifth of one pitcher's miss; the fourteen-season detection wall on college called-strike command; the catcher-assignment artifact as BIAS not noise; the declared-target miss log; the challenge-success selection effect; the ABS timeline for college | "Is the called-strike column on my staff report telling me anything?" "What CAN I measure inside one season?" "Should my pitcher spend the ABS challenge?" "Do I recruit a catcher for receiving with ABS coming?" **§1–§4 are closed-form and durable; §5 is a snippet-level field sweep and every magnitude in it is quarantined** |
| **`daily/2026-09-10-report.md`** | **NEW** | 2026-09-10 | Cycle 10 brief. **SEVENTH consecutive fully-blocked cycle**, and the first in which the gateway names its own denial. Opens the catcher; catches the search summariser **disagreeing with itself about the same source in one cycle** (F-312); runs the arithmetic self-check nobody had run on the corpus's own most population-matched finding (F-311) | "How big is the framing contamination in my command numbers?" "Why does more data make the catcher artifact worse?" "What does the biggest study on ACC/SEC pitchers actually buy you?" |
...
**Total corpus:** ~8,700 lines of research across **14** library files, **11** daily briefs, and one structured citation module. **314 registered findings.**
...
### The catcher — framing as a contaminant of command measurement — **added 2026-09-10**

| Question | Go to |
|---|---|
| Is the called-strike-rate column on my staff report telling me anything? | **F-307 — NO, and not for the reason you think.** A college starter generates ~150–360 framable takes a season; detecting a 5 pp change needs ~3,140 of them — **fourteen seasons.** Not a "bigger sample" problem: **the sample does not exist inside a career.** Delete the column or rename it "catcher + umpire + noise" |
| How big is the catcher contamination underneath that? | **F-308 — the size of the entire real signal, AND IT IS BIAS.** A 2.5 pp artifact on a two-catcher staff is ~70% of the best CSAA season in MLB (F-182). ⚠️ **It does not shrink with innings. Pooling seasons to "fix" F-307 makes it WORSE.** Second member of the F-287 family |
| How much is framing actually worth, in units a pitcher cares about? | **F-306 — ~14 pp of shadow-strike rate per INCH.** The whole best-to-worst MLB framing spread ≈ **1.2–1.6 in**, against a ~7 in elite / ~11–13 in average pitcher miss (F-183). **Large in runs because it is applied 1,600× a year; tiny in inches. No pitcher's development plan should contain a framing term** |
| Why is the borderline band so decisive? | **F-305 — the shadow annulus is LARGER IN AREA than the strike zone it borders** (by 5–17%). Pure geometry. ⚠️ Area is not probability; the pitch share is bracketed 12–20% (F-314) |
| Then what CAN I measure in one season? | **F-310 + `catcher-framing-command.md` §4 — INCHES from a DECLARED target.** ~200 charted pitches detects a 2-inch gain; the called-strike route needs ~21,000. ⚠️ **Say "several-fold fewer," never the ratio** (Dispute #22, generalised). ⚠️ Rests on F-197, which does not exist |
| Should my pitcher spend the ABS challenge? | **F-309 — almost never.** He is furthest from the pitch, with the strongest motivated reasoning and the least information. **And challenge success rate is mechanically ANTI-correlated with framing skill** — a leader in it is displaying a larger supply of mistakes against him. **General rule: before reading any success rate, ask who chose the denominator** |
| Do I recruit a catcher for receiving? | **F-313 — with a depreciation schedule.** SEC ran ABS at the 2026 tournament; **NCAA approved it for all divisions in 2027.** Structural fact corroborated across four outlets. ⚠️ **Every framing MAGNITUDE found this cycle is unverified — do not repeat 0.704, 0.565, 18.7 pp, 56.2%** |

### Command
| Sub-topic | Primary file | Findings |
|---|---|---|
| **Why called strikes cannot measure it (college)** | **`catcher-framing-command.md` §3** | **F-307, F-308, F-305** |
| **What to measure instead — declared-target miss log** | **`catcher-framing-command.md` §4** | **F-310, F-314** |
...
### What is CONTESTED — the twenty-three open disputes
...
| 23 | Is the college catcher-assignment artifact (F-308) worth acting on, given that the only clean fix is unavailable? | **OPEN — added 2026-09-10.** Biomechanist: the finding is **inert** — you cannot randomise catcher assignment, so the only action is "stop using the metric," which F-307 already required on variance grounds alone. Coach: **noise makes you uncertain, bias makes you confidently wrong** — a staff that "fixes" F-307 by pooling seasons *increases* the artifact while believing it has solved the problem, and F-308 is precisely the finding that forbids that repair. **Neither has yielded. The disagreement is about whether a finding whose only function is to forbid a repair counts as actionable.** Settled by observation, not analysis: **ask three pitching coordinators which repair they reached for when their command numbers looked noisy** |

> **⚠️ DISPUTE #22 WAS GENERALISED OUT OF SEQUENCING ON 2026-09-10.** The identical objection was raised independently against **F-310**, a new finding in a different topic reached by a different route, one cycle after it was first raised against F-295. **Two instances is a pattern, and #22 is now a STANDING METHODOLOGICAL RULE: whenever this program computes a detection ratio between two variables measured in different units, THE ORDERING IS THE OUTPUT AND THE RATIO IS NOT.** Say "several-fold fewer pitches," never the multiplier. Binds **F-295, F-310**, and everything of this shape that follows. *(One thing was defended and is new: F-306 supplies a conversion rate between the two units, and a ~3.5× gap survives the conversion — so the direction is not merely a unit artefact, but the headline number was, twice.)*
...
- ~~**Catchers, framing and the pitcher-catcher system**, despite framing contaminating command measurement (F-182).~~ **PARTIALLY OPENED 2026-09-10** — `library/catcher-framing-command.md`, F-305→F-310, F-313, F-314. **The MEASUREMENT-CONTAMINATION half is now covered and is source-independent arithmetic**: the shadow annulus exceeding the zone it borders, the ~14 pp-per-inch conversion, the fourteen-season detection wall on college called-strike command, the catcher-assignment artifact as bias rather than noise, the input-vs-outcome asymmetry, and the challenge-success selection effect. **Three limits: (a) opened in a SEVENTH consecutive blocked cycle, so nothing was verified at source and EVERY field magnitude — framing run values, the 18.7 pp gap, all challenge percentages — is quarantined (F-313); (b) two empirical inputs are bracketed rather than known, the shadow-take share (12–20%) and the miss-distance SD (4–6 in), and both are in-house quantities nobody publishes (F-314); (c) the recommendation in §4 measures a BULLPEN quantity and rests entirely on F-197, which does not exist (Dispute #20a).** **Still entirely uncovered: game-calling and pitch selection by the catcher, the pitcher-catcher relationship as a performance variable, and blocking/throwing as they affect a pitcher's willingness to throw a pitch in the dirt** — that last one is a genuine performance lever and nothing in the corpus touches it.
...
- **No published shadow-take count for a college pitcher-season, and no published miss-distance SD for an 85+ arm.** Added 2026-09-10 (F-314). F-307's detection table brackets the first at 12–20% of all pitches and F-310's power table brackets the second at 4–6 in; both conclusions survive their brackets, but neither number is known. Statcast's framing leaderboard is **MLB-only and there is no college analogue.** **Third and fourth entries in the pattern F-264, F-289 and F-295 established — and the pattern is now itself a finding about the field: THE NUMBERS THAT WOULD LET A COACH READ HIS OWN DATA ARE PRECISELY THE ONES NOBODY PUBLISHES, because they are nuisance parameters to a researcher and the whole answer to a practitioner.** A month of charted bullpens produces the miss SD.
- **No public discussion anywhere of the college-catcher confound in command evaluation.** Added 2026-09-10. Searched; nothing found. Second cycle running (cf. sequencing, 2026-09-09) in which the honest field report is *the industry is not arguing about this at all.* ⚠️ Recorded under the limits of a blocked cycle — cf. Known Correction #4.
...
- **Ten research days exist** (2026-08-12, 2026-08-13, 2026-08-20 verification pass, 2026-09-04 through 2026-09-10). Most of the corpus is still only two cycles deep.
- **🚨 ESCALATED 2026-09-10 — F-197 IS NO LONGER A TOPIC-SPECIFIC GAP; IT IS THE BINDING CONSTRAINT ON THIS PROGRAM'S ABILITY TO RECOMMEND ANYTHING.** Four consecutive cycles have now produced exactly one executable recommendation each, and **all four rest on a bullpen-to-game command transfer study that does not exist**: F-282's manufactured full count (09-07), the times-through-order monitoring block (09-08), the transition-matrix audit (09-09), and now the declared-target miss log (09-10). Each time the objection has been conceded and the recommendation demoted from a training intervention to a **measurement protocol**. **The corpus can now measure a great many things and cannot honestly claim that changing any of them changes a game.** (Dispute #20a.)
...
**And two more since 2026-09-10:**
14. **F-308** — **the second bias that does not shrink, and the first one that punishes the obvious repair.** F-287 established that more data can make an error more confident. F-308 goes further: a staff correctly told "your command sample is too small" (F-307) will reach for pooling — which shrinks the noise and leaves the catcher artifact untouched, tightening the number around the wrong answer. **Dispute #23 is about whether a finding whose only function is to FORBID A REPAIR counts as actionable. It is unresolved and it is a better argument than it looks.**
15. **F-311** — **nobody had run the two-line check on the corpus's own most population-matched finding.** F-010 has sat in the registry since day one carrying `R² = 0.29` and `RMSE = 2.70 mph` as separate facts. They are not independent: together they say the sample SD is 3.20 mph, so **the entire five-algorithm ensemble, with full in-game biomechanics on 322 ACC and SEC pitchers, beats guessing the team average by half a mile an hour — and 81% of what it knows, it knows from a tape measure.** **The lesson is not about that paper. It is that a registry entry can hold every number needed to undercut its own headline for a month before anyone multiplies them together.**
...
**⚠️ ADDED 2026-09-10 — the 2026-09-10 cycle (F-304, seventh consecutive block) produced eleven findings. EIGHT ARE PURE DERIVATION** — F-305, F-306, F-307, F-308, F-309, F-310, F-311 and the arithmetic in F-304 are closed-form geometry and power calculations, re-derivable in thirty lines, dependent on no source. **Three carry a flag:**

| F-ID | What needs watching | Status |
|---|---|---|
| **F-306** | The **18.7 pp best-to-worst framing gap** used to illustrate the inches conversion | **SNIPPET-LEVEL, from `calledthird.com` — a domain of unknown standing matching the content-farm profile of F-274/F-284. THE FINDING IS DELIBERATELY STATED AS A CONVERSION RATE (~14 pp per inch) SO THAT IT DOES NOT DEPEND ON THE GAP.** Quote the conversion; **never quote 18.7 pp** |
| **F-313** | Every framing and ABS **magnitude** — 0.704/0.565 runs per 100 innings, 18.7 pp, the SEC's "56.2% conversion," all challenge percentages, Bailey +25 / Quero −13, Carson Kelly's record | **SNIPPET/SUMMARY-LEVEL, NO PAGE OPENED, and two readings of the same source disagreed in one cycle (F-312). DO NOT REPEAT ANY OF THEM.** The **structural** ABS fact (SEC 2026 tournament; NCAA all divisions 2027) IS corroborated across four independent outlets and is safe to plan around. The **0.125 runs/strike** Statcast conversion is documented methodology and is the one framing input treated as safe |
| **F-311** | The **importance-to-variance apportionment** in the Super Learner self-check | **THE ARITHMETIC IS EXACT** — SD = RMSE/√(1−R²) = 3.20 mph and a 0.50 mph net gain follow necessarily from two figures already verified into the corpus at F-010. **The step from "81.2% of variable importance" to "kinematics carry a low-single-digit share of variance" is DIRECTIONAL, not exact — importance shares are not variance shares.** Say the direction. ⚠️ **The sample's ACTUAL MEAN VELOCITY is UNKNOWN**; ~89–91 mph is an inference |

**And a third extension to the F-284 rule (F-312), which is the cheapest one yet.** Two `WebSearch` queries this cycle, both resolving to the same Sports Info Solutions article, returned **mutually inconsistent league aggregates with no quoted snippet behind either** — catcher challenge success 59% vs 56%, batter 48% vs 50%, **moving in opposite directions**. These are totals over thousands of challenges and cannot drift that way in weeks. **NEW STANDING RULE: two summariser readings of the SAME source are a cheap in-cycle consistency test, and are now the standard check for any magnitude a blocked cycle wants to import. Re-query. If the numbers move, the numbers are not evidence. It costs one search.** ⚠️ **The finding was NARROWED ON THE SPOT by cross-examination (§6.4): the Carson Kelly record is WITHDRAWN as evidence** — a player's record is supposed to move between article dates, and reading it as drift would be the F-303 error in reverse. **F-312 stands on the aggregates only.** This is the first cycle in which F-303's lesson was applied prospectively rather than as a retraction.
