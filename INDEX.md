...
**Compiled 2026-08-17. Updated 2026-09-13.** Companion to [`FINDINGS.md`](FINDINGS.md), the flat registry of **336** findings (**F-326→F-336 added 2026-09-13** — the pitch clock and inter-pitch tempo; the corpus's SECOND same-night-detectable instrument, a recovery model that contradicts the industry's stated mechanism, and the scarce thing this corpus almost never gets: a genuine INTERVENTION on the exact variable — see §7; F-315→F-325 added 2026-09-12 — the running game; the corpus's FIRST topic whose lever is detectable in five pitches, plus one card rule and one sample-mismatch catch — see §7; F-304→F-314 added 2026-09-11 — catcher framing and the contamination of a command number; all derivation plus one rules fact, one queue-head lead and one new laundering channel — see §7; F-294→F-303 added 2026-09-09, sequencing arithmetic plus a PARTIAL RETRACTION of this program's own fabrication verdict — see §7; F-285→F-293 added 2026-09-08, arithmetic plus one citation correction; F-277→F-284 added 2026-09-07, all source-independent arithmetic; F-268→F-276 added 2026-09-06; F-260→F-266 added 2026-09-05; F-251→F-259 added 2026-09-04; **F-255, F-256 and F-258 are UNVERIFIED leads in quarantine — see §7**).

> ### 🚨 EGRESS HAS NOW FAILED **NINE TIMES IN A ROW** (2026-09-04, -05, -06, -07, -08, -09, -11, -12, -13) — AND A CYCLE WAS MISSED ENTIRELY (2026-09-10)
> **Zero primary sources opened across NINE consecutive cycles, and ten calendar days.** **RE-CONFIRMED 2026-09-13 through two independent channels before a topic was chosen, and a THIRD probe against `pmc.ncbi.nlm.nih.gov` mid-cycle returned the same refusal naming the domain (F-326)** — previously RE-CONFIRMED 2026-09-12 — `curl` returns `CONNECT tunnel failed, response 403` and `WebFetch` returns its own `{"error_type":"EGRESS_BLOCKED"}` naming the domain, for `en.wikipedia.org` and `arxiv.org` alike (F-315). `WebSearch` works; `WebFetch` and raw `curl` do not. **NEW 2026-09-11: `WebFetch` was tested INDEPENDENTLY of curl and returns its own explicit `{"error_type":"EGRESS_BLOCKED"}` naming the domain** — previous cycles inferred this from curl's behaviour; it is now directly confirmed, and both channels fail for the same reason. The proxy names the cause itself: `connect_rejected (organization policy)`, for **every** host including **`en.wikipedia.org` used as a control**. **This is an empty network allowlist in the remote execution environment. It is not an outage, not a paywall, and not a proxy fault — IT REQUIRES A HUMAN TO CHANGE** (F-277, F-285, F-304).
...
> **The standing brief's WORKING-domain list has been wrong NINE times and should be DELETED from the prompt, not amended.** ⚠️ **2026-09-13 tested `pmc.ncbi.nlm.nih.gov` — which the brief names as WORKING — directly. It was refused.**
...
> **⚠️ THE UNREAD VERIFICATION QUEUE NOW STANDS AT 31 ITEMS (+4 on 2026-09-13: PMID 27434082, the ONLY manipulated study of inter-pitch rest ever run — GET ITS SAMPLE'S MEAN VELOCITY FIRST; the Savant Pitch Tempo leaderboard and its inclusion rule; the two NCAA clock PDFs; and the Baseball America / Pitcher List 2023 tempo-velocity split. +3 on 2026-09-12: the Savant pitcher running-game-prevention page; PMID 22487194's full text, specifically whether its velocity difference was ever tested; and Driveline's Feb-2026 biomechanics-and-command article). Nine consecutive arithmetic-only cycles have produced ~81 findings, essentially all internally consistent and NONE checked against data.** That is the condition F-259 warns about, and **the next cycle with working egress should spend itself entirely on the queue rather than opening a new topic.** Queue head, **REORDERED 2026-09-11: arXiv `2609.03786` (KBO umpire count-bias, F-312)** — it would touch THREE areas in one page (a confound ~4× larger than the catcher term on every called-strike command metric; a third answer to Dispute #20b; the strongest live objection to F-283) — then arXiv `2601.11904` (pitch-pattern motifs, F-299/F-303), then *Scientific Reports* s41598-025-28142-y (peer-reviewed KBO/ABS), then Brill/Deshpande/Wyner (F-290, F-292), then the Kovash & Levitt minimax pair (F-302).
...
**Total corpus:** ~9,800 lines of research across **16** library files, **13** daily briefs, and one structured citation module. **336 registered findings.**
...
### What is CONTESTED — the twenty-four open disputes
...
- **Psychology beyond motor learning** — competitive anxiety, routines, focus under fatigue, the mental side of an outing. ⚠️ **Escalated 2026-09-13:** F-330 shows the extra seconds a pitcher wants before a big pitch are the physiologically CHEAPEST seconds available to him, which relocates their value to the procedural/psychological channel this corpus does not cover at all.
...
- **Thirteen research days exist** (2026-08-12, 2026-08-13, 2026-08-20 verification pass, 2026-09-04 through 2026-09-09, 2026-09-11 through 2026-09-13). Most of the corpus is still only two cycles deep.
