...
**Compiled 2026-08-17. Updated 2026-09-09.** Companion to [`FINDINGS.md`](FINDINGS.md), the flat registry of **303** findings (F-294→F-303 added 2026-09-09, sequencing arithmetic plus a PARTIAL RETRACTION of this program's own fabrication verdict — see §7; F-285→F-293 added 2026-09-08, arithmetic plus one citation correction; F-277→F-284 added 2026-09-07, all source-independent arithmetic; F-268→F-276 added 2026-09-06; F-260→F-266 added 2026-09-05; F-251→F-259 added 2026-09-04; **F-255, F-256 and F-258 are UNVERIFIED leads in quarantine — see §7**).

> ### 🚨 EGRESS HAS NOW FAILED **SIX TIMES IN A ROW** (2026-09-04, -05, -06, -07, -08, -09)
> **Zero primary sources opened across six consecutive cycles.** `WebSearch` works; `WebFetch` and raw `curl` do not. On 2026-09-07, 2026-09-08 **and again on 2026-09-09** the proxy logged `403 to CONNECT — policy denial` for **every** host including **`en.wikipedia.org` used as a control**. **This is an empty network allowlist in the remote execution environment. It is not an outage, not a paywall, and not a proxy fault — IT REQUIRES A HUMAN TO CHANGE** (F-277, F-285).
> **The standing brief's WORKING-domain list has been wrong SIX times and should be DELETED from the prompt, not amended.**
> **PROBE EGRESS BEFORE CHOOSING A TOPIC.** This instruction was added after cycle 4, was followed on 2026-09-08 and 2026-09-09, cost ninety seconds each time and determined the whole shape of the cycle. **KEEP IT.** If egress is down, pick a gap that arithmetic can close (F-266, F-278, F-286, F-295).
> **⚠️ THE UNREAD VERIFICATION QUEUE NOW STANDS AT 23 ITEMS. Six consecutive arithmetic-only cycles have produced ~50 findings, essentially all internally consistent and NONE checked against data.** That is the condition F-259 warns about, and the corpus is now deep enough into it that **the next cycle with working egress should spend itself entirely on the queue rather than opening a new topic.** Queue head, REORDERED 2026-09-09: **arXiv `2601.11904` (pitch-pattern motifs)** — it would test F-299 directly AND resolve the F-284/F-303 retraction in one page — then the Brill/Deshpande/Wyner TTOP paper (F-290, F-292), then the Kovash & Levitt / *Sports Economics Review* minimax pair (F-302).
...
**Total corpus:** ~8,300 lines of research across **13** library files, **10** daily briefs, and one structured citation module. **303 registered findings.**
