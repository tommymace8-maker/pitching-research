...
# 14 — PITCH SEQUENCING, PREDICTABILITY AND HITTER ANTICIPATION

**⚠️ EVERY ENTRY IN THIS SECTION (F-251 to F-258) IS SNIPPET-LEVEL. Added 2026-09-02, a cycle in which ALL direct egress was blocked — `WebFetch` and `curl` both returned 403 for every domain attempted, including every domain the run brief lists as working (`journals.sagepub.com`, `frontiersin.org`, `pmc.ncbi.nlm.nih.gov`, `ncbi.nlm.nih.gov/pmc`), plus `arxiv.org`, `nber.org`, `tangotiger.net`, `papers.ssrn.com` and a plain `example.com` control. `WebSearch` was the only working channel. NOT ONE SOURCE WAS READ AT SOURCE.** By this corpus's own rule these are **leads, not findings**, and the 2026-08-19 precedent (a snippet-only cycle that required the 2026-08-20 pass to fix two misreads) sets the expected error rate. Verification is registered as F-258. See `daily/2026-09-02-sequencing.md`.

### F-251 | THE ONLY EXPERIMENTAL MANIPULATION OF PITCH SEQUENCE FOUND A NULL ON ORDER AND A LARGE EFFECT OF ADVANCE INFORMATION
TOPIC: sequencing, pitch order, previous pitch, anticipation, advance information, timing error, hitter, experiment, manipulation, marker, lever
CLAIM: In the only study located that experimentally manipulated pitch-type sequence and pitch-type foreknowledge in hitters, the previous pitch's type did not affect impact timing, while simply telling the hitter what was coming reduced timing error strongly — separating "unpredictability" from "sequence order" as two different variables with two different answers.
NUMBERS: n = 26 HIGH SCHOOL baseball players. Pitching machine. Fastball 34.3 +/- 1.3 m/s = 76.7 +/- 2.9 mph; curveball 25.4 +/- 1.0 m/s = 56.8 mph; slowball 25.5 +/- 0.9 m/s = 57.0 mph. THREE CONDITIONS: Continuity (same pitch type five times consecutively, 15 trials); Random (pitch type not conveyed, 30 trials); Open (next pitch type conveyed in advance, 20 trials). Outcome = timing error from the difference between measured ball–bat impact location relative to the batter's body and optimal impact location. RESULTS: the (n-1)th pitch type did NOT affect impact timing, **p = 0.338**. Timing errors were fewer in Open than Random, **p < 0.001**. NO effect sizes retrieved. NO whiff, exit-velocity or run-value outcome was measured. Authors' stated mechanisms for increasing hitter timing error: having two or more pitch types; reducing fluctuation of the pitching motion; similar early ball trajectory between different pitch types — **ORDER IS ABSENT FROM THAT LIST.**
POPULATION: off_population_HS (hitters, not pitchers) — SAMPLE MISMATCH, DIRECTIONAL ONLY
EVIDENCE: EMERGING (single study, n = 26, off-population, machine-fed, timing-only outcome) — and SNIPPET-LEVEL, not read at source
CAUSALITY: INTERVENTION (hitter-side; sequence and foreknowledge both manipulated)
SOURCE: Kashiwabara et al. (2020), "Does the combination of different pitches and the absence of pitch type information influence timing control during batting in baseball?", PLOS ONE 15(3):e0230385; PMID 32182276; PMC7077830. NOT READ AT SOURCE 2026-09-02 — egress blocked. Open access; HIGHEST-PRIORITY VERIFICATION TARGET IN THE CORPUS.
COACHING: THIS IS THE FINDING THAT SPLITS THE SEQUENCING QUESTION IN TWO. Knowing the pitch helped the hitter a lot; learning it from the previous pitch helped him not at all. The information channel that "set him up with the fastball to get him with the slider" assumes exists DID NOT CARRY INFORMATION HERE. What the authors point at instead — arsenal breadth, delivery consistency, matched early trajectory — are all things a pitcher can actually change, and none of them is an ordering decision. FOUR REASONS NOT TO OVERQUOTE THIS: (1) a pitching machine has no delivery, so the expectancy channel was tested stripped of the arm-slot and torso cues a real pitcher provides — this isolates the sequence channel cleanly but limits generalization; (2) timing error is not an outcome; (3) a 57 mph breaking ball against a 77 mph fastball is a ~20 mph gap, far wider than an elite arsenal's, which may make late identification easy and suppress any sequence effect; (4) HS hitters, and Gray 2002 (F-252) reports the expectancy machinery STRENGTHENS with playing level — so this null may simply not describe an SEC hitter. DO NOT quote this null to a college pitcher as settled.
CONFIDENCE: low-medium — the design is genuinely a manipulation, which is rare and valuable, but the population is wrong, the apparatus is impoverished, and nothing was verified at source
SEE ALSO: F-252, F-253, F-254, F-256, F-163, F-195, F-258

### F-252 | THE COUNTER-EVIDENCE: A VR STUDY FOUND PREVIOUS-3-PITCH HISTORY DID MOVE SWING ACCURACY — n = 6
TOPIC: sequencing, anticipation, expectancy, Markov, virtual reality, previous three pitches, pitch count, swing accuracy, contradiction
CLAIM: A two-state Markov expectancy model driven by the types of the previous three pitches fit college hitters' swing accuracy in a VR batting task, with reported significant effects of both pitch history and count — directly contradicting the lag-1 null, on a sample of six.
NUMBERS: n = 6 experienced COLLEGE baseball players. VR batting simulation. Two-state Markov model: batter switches between expectancy states under transition rules; probability of expecting a given pitch computed from **the types of the previous THREE pitches**; that probability predicts swing accuracy. REPORTED: model provided a good fit; **the history of the previous 3 pitches AND the pitch count had significant effects on accuracy**; estimated model parameters were **highly correlated with playing level**. NO effect sizes, NO test statistics, NO p-values retrieved. Companion paper reports batters were influenced by pitch history and count, that pitch-speed variance produced predictable spatial errors, and that spatial accuracy was worse than temporal accuracy.
POPULATION: NCAA (hitters) — small
EVIDENCE: WEAK (n = 6; a free-parameter model fit to six subjects; "good fit" is not a hypothesis test; a correlation with playing level across six people has ~4 df) — and SNIPPET-LEVEL
CAUSALITY: INTERVENTION (simulated, hitter-side)
SOURCE: Gray R. (2002), "'Markov at the Bat': A Model of Cognitive Processing in Baseball Batters," Psychological Science 13(6):542-547, PMID 12430839; and Gray R. (2002), "Behavior of College Baseball Players in a Virtual Batting Task," J Exp Psychol Hum Percept Perform 28(5):1131-1148, PMID 12421060. NOT READ AT SOURCE 2026-09-02 — egress blocked. Sample size and design come from search summaries only; OPERATING RULE (D) APPLIES — this corpus has already seen a search summary manufacture a phantom n (F-242).
COACHING: DO NOT PRESENT THE SEQUENCE QUESTION AS SETTLED IN EITHER DIRECTION. F-251 and F-252 are the only two experimental manipulations that exist and they disagree. FOUR AVAILABLE RECONCILIATIONS, most plausible first: (1) DEPTH OF HISTORY — Gray modeled a state built from THREE prior pitches; the null tested ONE. An expectancy requiring three pitches to load is invisible to a lag-1 test, and this is testable; (2) SKILL — Gray's own parameters-correlate-with-level result implies the machinery grows with skill, which would make an HS null uninformative about the hitters Tommy's pitchers face; (3) outcome measure (swing accuracy vs impact-location timing error); (4) apparatus (rendered pitcher with delivery cues vs machine with none). NEITHER STUDY IS STRONG. They are simply the only two.
CONFIDENCE: low — n = 6, model-fit evidence, nothing verified at source
SEE ALSO: F-251, F-253, F-256, F-258

### F-253 | AT 12.4 MILLION PITCHES, SEQUENCE DIVERSITY DID NOT PREDICT ERA OR WINS AND TOP MOTIFS DID NOT SEPARATE HITS FROM OUTS
TOPIC: sequencing, motifs, Shannon entropy, inverse Simpson, ERA, wins, hit, out, Zipf, null, Statcast, large sample
CLAIM: The largest public analysis of pitch sequencing ever conducted reports that pitcher-level sequence diversity has no decisive association with ERA or wins, and that the most frequently used motifs do not discriminate hit from out — while confirming that motif usage itself is highly structured and non-random.
NUMBERS: ~12,400,000 Statcast pitch records, **2008-2025 MLB regular seasons**. Two scales: MACROSCALE — Shannon entropy and inverse Simpson index of motif usage tested against **earned run average and wins**; MICROSCALE — hit vs out frequencies compared across pitch-pattern motifs. AUTHORS' OWN ABSTRACT LANGUAGE (close to verbatim, snippet): motif-based sequence diversity at pitcher level as quantified by Shannon entropy "exhibits less decisive association with conventional performance measures such as ERA or wins"; "the most highly ranked motifs do not contribute to discriminating between HIT and OUT outcomes," indicating "inherent limitations in using motif-based diversity measures alone as independent variables for predicting seasonal pitching performance." POSITIVE FINDING: motif usage exhibits stable, non-random organization following **Zipf's law**. **NO EFFECT SIZE, NO CORRELATION COEFFICIENT, NO p-VALUE WAS RETRIEVABLE.**
POPULATION: MLB
EVIDENCE: EMERGING — enormous n, but weak dependent variables, season-level aggregation, and NO retrievable effect size. SNIPPET-LEVEL.
CAUSALITY: CROSS_SECTIONAL
SOURCE: "Structure of Pitch-Pattern Motifs in Major League Baseball," arXiv:2601.11904; published Journal of the Korean Physical Society, DOI 10.1007/s40042-026-01641-5. NOT READ AT SOURCE 2026-09-02 — arxiv.org and link.springer.com both blocked.
COACHING: DO NOT QUOTE THIS AS "r = 0" OR "NO ASSOCIATION." "Less decisive association" is the authors' own hedge, not a coefficient, and quoting it harder than they wrote it is exactly the failure mode of F-240. THE COACH'S OBJECTION IS PARTLY VALID AND WAS CONCEDED: ERA and wins aggregated to a season would wash out a real effect that lives in specific counts against specific hitters, and wins is close to meaningless at pitcher level. THE PART THAT SURVIVES THAT OBJECTION: the microscale arm is at MOTIF level, not season level, and still found no hit/out discrimination. ALSO NOTE this is a statistical-physics paper whose headline is Zipf's law; outcomes are secondary, which is a reason to be careful about how hard its null was tested. NOBODY HAS RUN THE WITHIN-PITCHER VERSION (F-257).
CONFIDENCE: medium on the direction, low on the magnitude — the magnitude was never published in retrievable form
SEE ALSO: F-251, F-254, F-255, F-257, F-094, F-163

### F-254 | PITCHERS ALTERNATE PITCH TYPES TOO MUCH, AND REPEATING A PITCH IS NOT PUNISHED — two independent lines, both contested
TOPIC: sequencing, serial correlation, minimax, game theory, repeat pitch, same pitch twice, alternation, predictability, usage
CLAIM: Two independent analyses converge on the opposite of coaching folklore: MLB pitchers show negative serial correlation in pitch calling — they alternate more than randomization would — and throwing the same pitch consecutively performs at or above the average sequencing result in nearly every category.
NUMBERS: KOVASH & LEVITT (2009), **>3,000,000 pitches** (plus ~125,000 NFL play calls). Two claims: pitchers throw **too many fastballs**, and there is **negative serial correlation in play calling** in both sports — i.e. too much alternation. Back-of-envelope: correcting these errors worth **"as many as two additional victories a year"** to a franchise. **THE CRITIQUE:** Birnbaum (Sabermetric Research, Sept 2009) and others argue (a) the correct equilibrium test is to adjust until OPS-after-fastball equals OPS-after-non-fastball and **the paper did not make that comparison**, instead evaluating **only pitches that ended the at-bat** — a biased subset; (b) **OPS was reportedly computed incorrectly, undervaluing walks**, which would inflate the apparent effectiveness of breaking pitches — precisely the direction needed to produce the "too many fastballs" conclusion; (c) game theory cannot deliver the right proportion without probably-wrong assumptions. — FANGRAPHS COMMUNITY RESEARCH, "The Effects of Repeating Pitches on Pitcher Success Rate," **2023 Trackman data**: throwing the same pitch twice **outperforms the average results of sequencing in every area except fastball whiffs**, and each pitch type is **about as effective in isolation as when thrown one to three times in a row**. NO sample size retrieved, NO control for pitcher quality or count.
POPULATION: MLB
EVIDENCE: WEAK on magnitude, EMERGING on direction — Kovash & Levitt has a substantive published critique of its baseball arm; the repeat study is grey literature with no n and no controls. SNIPPET-LEVEL.
CAUSALITY: CROSS_SECTIONAL
SOURCE: Kovash K. & Levitt S.D. (2009), "Professionals Do Not Play Minimax: Evidence from Major League Baseball and the National Football League," NBER Working Paper No. 15347; Birnbaum P. (Sept 2009), "A game theory study on pitch selection," Sabermetric Research; FanGraphs Community Research, "The Effects of Repeating Pitches on Pitcher Success Rate." NOT READ AT SOURCE 2026-09-02 — nber.org, tangotiger.net (PDF host) and community.fangraphs.com all blocked.
COACHING: "NEVER THROW THE SAME PITCH TWICE" IS FOLKLORE AND SHOULD BE RETIRED AS AN INSTRUCTION. Two independent lines say pitchers already alternate too much and that repetition is not punished. QUOTE THE SERIAL-CORRELATION HALF, NOT THE MAGNITUDE HALF: the "too many fastballs" claim and the "two wins a year" figure both rest on an OPS calculation the critique says undervalues walks, and on an at-bat-ending subset. **DO NOT SAY "TWO WINS."** THE CONFOUND THAT RUNS THE OTHER WAY, and nobody has controlled for it: pitchers repeat a pitch when it is working and when the count permits, so selection makes repetition look good. The honest position is that the folklore is unsupported, NOT that repetition is a positive strategy.
CONFIDENCE: medium that the folklore is wrong; low on any magnitude
SEE ALSO: F-253, F-255, F-256

### F-255 | THE EXPLOITABLE PREDICTABILITY IS REAL AND SMALL — roughly 0.23 bits, ~71% on a binary guess, and it grows as the pitcher falls behind
TOPIC: predictability, entropy, information, bits, pitch prediction, machine learning, count, sequencing, instrument
CLAIM: Sequencing leaks a small, measurable amount of information about the next pitch — on the order of a tenth of the hitter's total uncertainty — and the leak is largest when the pitcher is behind in the count.
NUMBERS: BRENNAN (Baseball Prospectus): **mean information given up through sequencing ~0.23 bits, range 0.15-0.35 bits**; pitchers become **more predictable the further they fall behind in the count**. CONTEXT ARITHMETIC (computed here, 2026-09-02): with four pitch types maximum uncertainty is 2 bits, so ~0.23 bits is roughly **a tenth of the hitter's uncertainty**. — NEXT-PITCH PREDICTION MODELS, **numbers from DIFFERENT studies and NOT poolable**: binary fastball vs non-fastball **~71%** accuracy in one wearable-sensor study; 3-class (fastball / offspeed / breaking) **61.3%**; in a per-pitcher study **42 of 70 pitchers beat their own baseline on the binary task, only 12 of 70 on the 3-class task**. ONE OUTLIER TO DISCARD: an XGBoost report claiming **93.3% accuracy, macro-F1 0.873** — **30 points above every other result; almost certainly leakage or a high-base-rate per-pitcher setup. DO NOT QUOTE IT.** Feature-importance rankings consistently place **previous pitch type** among the top predictors alongside release speed and spin rate.
POPULATION: MLB
EVIDENCE: WEAK — mixed grey literature and student ML projects, heterogeneous designs, no common benchmark. SNIPPET-LEVEL.
CAUSALITY: CROSS_SECTIONAL
SOURCE: Brennan P., Baseball Prospectus (sequencing entropy); assorted next-pitch prediction reports located 2026-09-02 via search only. NOT READ AT SOURCE — baseballprospectus.com and medium.com blocked.
COACHING: THIS IS THE SIZE OF THE PRIZE, AND IT BOUNDS EVERYTHING ELSE ON THIS TOPIC. ~0.23 bits and a ~71% binary guess is a real edge and a small one — and note WHERE it lives: in a pitcher's **overall tendencies and count behaviour**, not in any individual clever two-pitch combination. That is a direct argument for auditing usage by count (the recommended action) over arguing about order. THE PRACTICAL INSTRUMENT: per-count pitch-type usage is computable from any Trackman log with no new data collection. **SAMPLE SIZE TO USE IT: a usage rate is a proportion, so pinning one to +/-10 percentage points at 95% confidence needs ~96 pitches IN THAT COUNT BUCKET.** A college starter throws ~1,000-1,400 pitches a season across 12 buckets, so **only the five or six most common counts yield usable estimates, and only over a full season.** One bullpen tells you nothing. Say this to the athlete before starting.
CONFIDENCE: low-medium — the 0.23 bits figure is a single grey-literature number and the prediction accuracies are not comparable to each other
SEE ALSO: F-253, F-254, F-257, F-171, F-186

### F-256 | THE TIMES-THROUGH-THE-ORDER PENALTY: ARSENAL BREADTH MITIGATES IT, BUT THE DISCONTINUITY ITSELF IS WEAKLY SUPPORTED AFTER CONFOUNDER ADJUSTMENT
TOPIC: times through the order, TTOP, familiarity, fatigue, arsenal breadth, fastball frequency, wOBA, discontinuity, Bayesian, confounding
CLAIM: The received view — that pitchers decline through the order because of hitter familiarity, and that a broad arsenal protects against it — rests on an unadjusted discontinuity that a confounder-adjusted Bayesian reanalysis found little evidence for, though that reanalysis did not test the arsenal-breadth interaction.
NUMBERS: LICHTMAN (SABR / Baseball Prospectus; **data 2002-2012**, Baseball Info Solutions pitch types via FanGraphs): pitchers who throw mostly fastballs **lose 47 points of wOBA** against by the third time through the order; those with much lower fastball frequency **lose only 18 points**. Interpretation offered: the penalty is **familiarity, not fatigue**. — BRILL, DESHPANDE & WYNER (JQAS 2023; arXiv:2210.06724): after adjusting for **batter quality, pitcher quality, handedness and home-field advantage**, there is **little evidence of a strong discontinuity in pitcher performance between times through the order**; existing approaches **cannot disentangle continuous within-game evolution from discontinuities between successive times through**; stated practical implication — **the start of the third time through should not be treated as a special cutoff for pulling a starter.** No effect sizes retrieved from either side.
POPULATION: MLB
EVIDENCE: EMERGING and CONTESTED. SNIPPET-LEVEL.
CAUSALITY: CROSS_SECTIONAL (both)
SOURCE: Lichtman M., "The penalty for pitchers going through the batting order" and "Pitch types and the times through the order penalty," SABR / Baseball Prospectus; Brill R., Deshpande S.K. & Wyner A.J. (2023), "A Bayesian analysis of the time through the order penalty in baseball," Journal of Quantitative Analysis in Sports, DOI 10.1515/jqas-2022-0116, arXiv:2210.06724. NOT READ AT SOURCE 2026-09-02 — sabr.org, degruyterbrill.com, arxiv.org all blocked.
COACHING: STATE THE BRILL RESULT PRECISELY OR IT WILL BE MISREPORTED. They do NOT claim pitchers fail to get worse as a game goes on. They claim **THE JUMP AT EACH NEW TIME THROUGH IS NOT WELL SUPPORTED** — and that distinction carries the mechanism, because **familiarity predicts a discontinuity** (the hitter's information resets upward each plate appearance) while **fatigue predicts a smooth decline**. If the discontinuity is not there, the cleanest published evidence for the familiarity mechanism weakens, and so does the strongest argument that "he's already seen it" is a large effect. BOTH RESULTS CAN BE TRUE SIMULTANEOUSLY: no clean discontinuity overall, and a real conditional advantage for a broad arsenal — Brill et al. did not test the breadth interaction. STANDING ON THE AUTHORS: this corpus already carries Ludwig/Brill/Wyner (arXiv:2508.19184, xCTRL) as verified-clean; same group, same posture of distrusting effects that survive only without confounder adjustment, which is the posture this program adopted for itself (F-240).
CONFIDENCE: medium — both legs are real published work; neither magnitude is verified and the two have never been reconciled in print
SEE ALSO: F-253, F-257, F-094, F-240

### F-257 | THE SEQUENCING LITERATURE IS ENTIRELY BETWEEN-PITCHER, WHICH F-094 SAYS IS THE WRONG DIRECTION — and the within-pitcher study is computable in an afternoon
TOPIC: sequencing, predictability, within-pitcher, between-pitcher, absence, gap, study design, trainability, stability, restriction of range
CLAIM: Every sequencing and predictability result in the literature is a between-pitcher cross-section; nobody has asked whether the same pitcher performs better in his own less-predictable outings, and nobody has established that predictability is even stable within a pitcher — so its status as a trainable variable is entirely unknown.
NUMBERS: ABSENCE SEARCH 2026-09-02 covering pitch sequencing, pitch order, previous-pitch effects, pitch-call entropy, predictability, next-pitch prediction, times-through-the-order, hitter anticipation, and commercial pitch-calling products. FOUND ABSENT: **(1) no study manipulating pitch sequence with professional or D1 HITTERS** — the two manipulations that exist are n = 26 high school (F-251) and n = 6 college-in-VR (F-252); **(2) no sequencing analysis controlling for pitcher quality as a confound** — every observational result is contaminated by "good pitchers do X"; **(3) no published outcome validation of any commercial sequencing product**, Driveline's Paint Mixer / "buyback effect" included; **(4) no study of pitch sequencing at the college level at all** — everything is MLB Statcast or laboratory; **(5) no test of whether a pitcher can be TRAINED to be less predictable**; **(6) no year-over-year stability estimate for per-count pitch-type entropy**; **(7) no within-pitcher analysis** regressing per-outing run value on that outing's entropy with pitcher fixed effects. Items (6) and (7) are computable from public Statcast data with no new collection.
POPULATION: N/A — absence
EVIDENCE: ESTABLISHED as an absence (searched deliberately, not assumed) / NONE WHATSOEVER on trainability
CAUSALITY: MECHANISM
SOURCE: daily/2026-09-02-sequencing.md §4f and §8; absence search 2026-09-02. Search-channel only — egress blocked, so the absence is established against SEARCH INDEXES, not against journal tables of contents. Weaker than the F-250 absence, which had source access.
COACHING: THE CORPUS'S OWN CENTRAL METHODOLOGICAL FINDING CONDEMNS THIS LITERATURE'S DESIGN. F-094 — within-athlete R2 = 0.957 vs between-athlete R2 = 0.076 for velocity and torque — says between-pitcher nulls are close to uninformative about within-athlete levers. **Every sequencing result on this list, positive or null, is between-pitcher.** Add the program's standing restriction-of-range problem (Luera 2020, r = .17-.29 in an all-hard-throwing sample) and the elite-population versions of these effects are probably smaller still than the published numbers. TWO QUESTIONS, IN ORDER, AND THE FIRST ONE CAN KILL THE TOPIC: **(a) is per-count pitch-type entropy even stable year over year within a pitcher?** If not, it is noise and the whole subject collapses. **(b) Do pitchers do better in their own higher-entropy outings?** Same category as the release-speed-SD-vs-vertical-miss study already flagged as the corpus's cheapest high-value question (F-173). UNTIL (a) IS ANSWERED, "be less predictable" IS A MARKER-SHAPED INSTRUCTION AND MUST NOT BE SOLD AS A LEVER — the same error as stride length (F-043) and extension (F-250).
CONFIDENCE: high on the absences; the search channel is the limitation, not the reasoning
SEE ALSO: F-094, F-043, F-250, F-173, F-251, F-253, F-258

### F-258 | A NEW HAZARD CLASS — DIRECTIONAL INVERSION OF A REAL SOURCE. A content farm is circulating the 12.4M-pitch paper's conclusion backwards, and citation-checking would pass it
TOPIC: content farm, hazard class, misattribution, inversion, verification, motifs, sequencing, marketing, blocklist, epistemics
CLAIM: Two commercial sites are citing the 12.4-million-pitch motifs paper — correct title, correct sample size, real journal — for a conclusion that is the reverse of what the paper reports, producing a claim that survives every verification step this program currently runs except reading the abstract.
NUMBERS: SITES: **atsstats.com** (a sports-betting stats site) and **mkdcbaseball.com** ("Pitch Arsenal & Design Guide 2026"). **NOTE: mkdcbaseball.com WAS ALREADY ON THIS CORPUS'S BLOCKLIST from Sweep 1 (2026-08-12) — it is a repeat offender, and the blocklist did its job as a warning while the claim still reached this cycle through a search summary. atsstats.com is new.** CIRCULATED CLAIMS: "A pitcher with average stuff and elite sequencing outperforms a pitcher with elite stuff and linear sequencing"; and "Recent academic research analyzing **12.4 million** Statcast pitches identifies pitch sequences as a language-like structure where **the effectiveness of a pitch is defined by the pitches preceding it**." WHAT THE CITED PAPER ACTUALLY REPORTS (F-253): sequence diversity "exhibits less decisive association with conventional performance measures such as ERA or wins," and "the most highly ranked motifs do not contribute to discriminating between HIT and OUT outcomes." **THE 12.4 MILLION FIGURE IS CORRECT. THE CONCLUSION IS REVERSED.**
POPULATION: N/A — epistemics
EVIDENCE: ESTABLISHED (the inversion is directly checkable against the paper's own abstract language)
CAUSALITY: N/A
SOURCE: daily/2026-09-02-sequencing.md §4a. Field sweep 2026-09-02.
COACHING: **ADD `atsstats.com` TO THE CONTENT-FARM BLOCKLIST; `mkdcbaseball.com` IS ALREADY ON IT (Sweep 1) AND IS NOW A NAMED REPEAT OFFENDER.** AND REGISTER THE HAZARD CLASS, because it is not a fabrication and it is not an AI hallucination: **the citation is real, the sample size is real, the journal is real, and the direction is flipped.** It defeats citation-checking by construction. This is structurally identical to correction #2 of 2026-08-13 — the changeup framing, which also inverted its source — and it is exactly what operating rule (A) exists to catch: *does the source support THIS sentence, or a weaker one, or the opposite one?* **THE COUNTERMEASURE IS THE ABSTRACT'S OWN VERBS, NOT THE PMID.** DO NOT REPEAT "elite sequencing beats elite stuff" TO ANY ATHLETE OR STAFF MEMBER. — SECOND, AND SEPARATE: **THIS ENTIRE SECTION (F-251 to F-258) IS UNVERIFIED.** All direct egress was blocked on 2026-09-02; nothing was read at source. VERIFICATION PRIORITY ORDER: (1) **PLOS ONE 15(3):e0230385 / PMC7077830** — confirm n = 26, HS level, p = 0.338 on the (n-1) pitch, p < 0.001 on Open vs Random, and that "timing error" is what F-251 says it is; (2) **arXiv:2601.11904 / JKPS** — obtain an actual effect size, because "less decisive association" is currently doing far too much work; (3) **Gray 2002** — confirm n = 6 and whether the previous-3-pitch effect is a hypothesis test or a model-fit statement; (4) Kovash & Levitt's baseball numbers and Birnbaum's OPS critique; (5) Lichtman's 47/18 wOBA split and the Brill et al. adjusted result.
CONFIDENCE: high on the inversion; high on the fact that this cycle is unverified
SEE ALSO: F-253, F-240, F-241, F-242, F-232, F-251

---

# 18 — CYCLE 2026-09-03: RECONCILIATION, AND WHAT THE 09-02 SWEEP DID NOT REACH

**Added 2026-09-03.** This cycle opened by selecting "pitchability / sequencing" from the `INDEX.md` §5 gap list — **and then discovered the topic had been covered the previous day.** The account of why is F-259, and it is the most important entry in this section. Everything else here is the residue: the items the 2026-09-02 sweep did not reach, plus one citation conflict between the two cycles.

**Same standing caveat as §17: SNIPPET-ONLY.** Egress was again totally blocked — third consecutive cycle. Nothing below was read at source.

### F-259 | ⚠️ STRUCTURAL — the vault is fragmenting across unmerged branches, and two cycles have already collided on F-IDs
TOPIC: methodology, process, git, branches, corpus integrity, duplication, F-ID collision, main, epistemics, standing hazard
CLAIM: Research cycles are pushing to branches that are never merged into `main`, so each new session clones `main`, sees a stale vault, and re-researches covered ground — and two cycles have already independently assigned the same F-IDs and the same dispute numbers to different findings.
NUMBERS: STATE OF THE REMOTE AS OF 2026-09-03: `main` at commit 2253801 ("Verification pass on the extension report", 2026-08-20). TWO UNMERGED BRANCHES, each forked directly from `main`, neither merged into it nor into each other: `daily/2026-09-01-hitter-perception` (1 commit, +117 lines FINDINGS.md, new `library/hitter-perception.md`) and `research/2026-09-02-sequencing` (1 commit, +104 lines FINDINGS.md, new `library/pitchability-sequencing.md`). THE COLLISIONS, exact: the 09-01 branch assigns F-252 through F-259; the 09-02 branch assigns F-251 through F-258. **F-252 to F-258 — SEVEN IDs — are assigned to DIFFERENT FINDINGS on the two branches.** Example: on 09-01, F-257 is "the ABS challenge system is live in MLB"; on 09-02, F-257 is "the sequencing literature is entirely between-pitcher." DISPUTE NUMBERS COLLIDE TOO: both branches add a **#17**, with different content — 09-01's is "Is velocity VARIABILITY a command liability or a timing weapon?", 09-02's is "Is pitch-sequence ORDER a lever, a marker, or nothing?". THE DUPLICATION COST, measured on this cycle: the 2026-09-03 session cloned `main`, read an `INDEX.md` whose §5 still listed pitchability as having "nothing in the corpus covering it," and spent a full research pass re-deriving a topic the 09-02 cycle had already covered — in several places better (09-02 retrieved p = 0.338 and p < 0.001 from the PLOS ONE manipulation and n = 6 for Gray; the 09-03 pass retrieved neither).
POPULATION: not applicable
EVIDENCE: ESTABLISHED (directly observed in the repository; `git ls-remote`, `git diff --stat` and branch inspection, 2026-09-03)
CAUSALITY: not applicable
SOURCE: Direct inspection of github.com/tommymace8-maker/pitching-research, 2026-09-03. `daily/2026-09-03-report.md` §1.
COACHING: THIS IS A CORPUS-INTEGRITY PROBLEM, NOT A GIT INCONVENIENCE, AND IT GETS WORSE EVERY CYCLE. The registry's own first rule is "numbering is stable — never reuse or renumber an F-ID," and that rule has already been broken seven times, by two cycles that could not see each other. THE MECHANISM IS SELF-REINFORCING: every unmerged branch makes `main` staler, which makes the next clone's `INDEX.md` more wrong about what is covered, which makes the next duplication more likely. THE 09-03 CYCLE'S RESPONSE, and the precedent it sets: it did NOT fork a fourth copy from `main`. It rebased onto `research/2026-09-02-sequencing` (the most recent cycle), renumbered its own findings to F-259+, deleted the portions that merely restated 09-02, and kept only what was additive. **A cycle that discovers its topic already covered should stack on the covering branch and contribute the residue — not publish a parallel account.** WHAT REQUIRES A HUMAN DECISION AND WAS DELIBERATELY NOT DONE AUTONOMOUSLY: merging the 09-01 branch, which requires renumbering seven of its F-IDs and one dispute number. Renumbering a published ID violates the registry's stated convention, so which branch keeps F-252 to F-258 is the owner's call. **Until that is resolved, F-252 to F-258 are AMBIGUOUS IDENTIFIERS and must always be cited with their branch.**
CONFIDENCE: high — directly observed
SEE ALSO: F-240, F-241, F-242, F-260

### F-260 | ⚠️ CITATION CONFLICT between two cycles — the first author of PLOS ONE 15(3):e0230385
TOPIC: citation hygiene, verification, correction candidate, PLOS ONE, Kidokoro, Kashiwabara, sequencing, attribution
CLAIM: The 2026-09-02 and 2026-09-03 cycles independently searched the same PLOS ONE paper and returned different author names for it; one of the two is wrong and neither could open the source.
NUMBERS: THE PAPER, identically identified by both cycles: PLOS ONE **15(3):e0230385**, PMID **32182276**, PMC7077830, DOI 10.1371/journal.pone.0230385, "Does the combination of different pitches and the absence of pitch type information influence timing control during batting in baseball?" — n = 26 high-school hitters, pitching machine, FB 34.3 +/- 1.3 m/s, three conditions (Continuity 15 / Random 30 / Open 20 trials). **THE TWO CYCLES AGREE ON EVERY IDENTIFIER, EVERY NUMBER AND EVERY RESULT. THEY DISAGREE ON THE AUTHORS.** 2026-09-02 (F-251, `library/pitchability-sequencing.md` §2.1) records "**Kashiwabara et al.**" 2026-09-03 recorded "**Kidokoro S, Matsuzaki Y, Akagi R**" — sourced from a search summary that named a **Shibaura Institute of Technology institutional-repository page** and listed the three authors in full with given names ("Kidokoro, Shuji; Matsuzaki, Yuji; and Akagi, Ryota"). NEITHER CYCLE OPENED THE PAPER. PLOS ONE is fully open access, so this is resolvable in one click the moment egress returns.
POPULATION: not applicable
EVIDENCE: UNVERIFIED — an unresolved conflict between two snippet-level readings
CAUSALITY: not applicable
SOURCE: `library/pitchability-sequencing.md` §2.1 (2026-09-02) vs the 2026-09-03 search pass. Neither read at source.
COACHING: THE 09-03 READING IS THE BETTER-EVIDENCED ONE AND IS STILL NOT PROOF. An institutional repository page that lists three authors with full given names is a stronger provenance than a bare "et al.", and the third author (Akagi, Shibaura Institute of Technology) is consistent with a Japanese biomechanics group publishing on batting timing. BUT THE 09-02 CYCLE MAY HAVE SEEN SOMETHING THIS ONE DID NOT, AND ITS READING OF THIS PAPER WAS OTHERWISE MORE COMPLETE — it retrieved p = 0.338 and p < 0.001, which the 09-03 pass did not. **DO NOT SILENTLY OVERWRITE EITHER NAME.** Both are recorded, dated, and flagged. WHY THIS MATTERS OUT OF PROPORTION TO ITS SIZE: this corpus's single most-repeated lesson is that a correct identifier does not certify the sentence attached to it (F-240, and the directional-inversion hazard the 09-02 cycle registered as F-258 on its branch). **Here is the same lesson at the smallest possible scale — a PMID, a DOI, a sample size and two p-values all agreed on by two independent passes, with the author line still in dispute.** ACTION: first item to resolve when egress returns. One click.
CONFIDENCE: medium that Kidokoro/Matsuzaki/Akagi is correct; high that the conflict is real and unresolved
SEE ALSO: F-240, F-259, F-251

### F-261 | The minimax claim has been REVERSED TWICE in 2024 — pitchers may not be exploitably predictable after all
TOPIC: pitchability, sequencing, game theory, minimax, Nash equilibrium, serial correlation, predictability, reversal, pitch location
CLAIM: Two independent 2024 analyses report that the two minimax predictions Kovash & Levitt found violated — payoff equality across actions, and absence of serial correlation — actually hold for the majority of players.
NUMBERS: PAPER 1: "Professionals do play Minimax: Revisiting the Nash equilibrium in Major League Baseball" (2024). Reported: "the hypotheses of equality of payoffs across actions and the absence of serial correlation hold for the majority of players." Reported deviations lie ELSEWHERE — batters exhibit LOWER SWING RATES than theory predicts; pitchers throw INSIDE THE STRIKE ZONE MORE OFTEN than expected. Method notes: examines PITCH LOCATION rather than pitch type; models BOTH players; uses machine learning to classify whether swung-at pitches were in or out of the zone. **AUTHORS NOT RETRIEVABLE. JOURNAL NOT IDENTIFIED.** Only identifier obtained: ScienceDirect PII **S2773161824000168**. No sample size, no seasons, no test statistics. PAPER 2: **White DR & Smith BO (2024), "Changing It Up: Determining the Nash Equilibria for Major League Baseball Pitchers," American Behavioral Scientist, DOI 10.1177/00027642241235829.** No numerical results retrievable at all. ⚠️ ATTRIBUTION RISK, FLAGGED NOT RESOLVED: the "pitch location rather than pitch type" and "machine learning for in/out of zone" descriptions arrived in a search summary covering BOTH papers and may have been conflated between them.
POPULATION: MLB
EVIDENCE: EMERGING — SNIPPET-ONLY, NOT SOURCE-VERIFIED, WITH A KNOWN ATTRIBUTION RISK (two independent 2024 sources pointing the same direction; one has no retrievable author; neither has a retrievable number)
CAUSALITY: CROSS_SECTIONAL
SOURCE: (1) ScienceDirect PII S2773161824000168 (2024) — AUTHORS AND JOURNAL UNVERIFIED. (2) White DR, Smith BO (2024), American Behavioral Scientist, DOI 10.1177/00027642241235829. Neither opened — sciencedirect.com and journals.sagepub.com both EGRESS_BLOCKED 2026-09-03.
COACHING: THIS CHANGES THE STANDING OF F-254 ON THIS BRANCH. The over-alternation result is not merely "contested" — it has been directly re-tested twice in one year, both times against it. COMBINED WITH BIRNBAUM'S OBJECTION (already registered): computing OPS on plate appearances THAT END ON A GIVEN PITCH is a biased read, and the correct object is all outcomes ON OR AFTER the pitch — which, if right, dissolves the "too many fastballs" leg by itself. **TREAT "HE'S TOO PREDICTABLE, HE ALTERNATES TOO MUCH" AS AN UNPROVEN DIAGNOSIS AT THE ELITE LEVEL, NOT A DEFAULT.** THE PORTABLE IDEA, and it is the more useful half of the 2024 work regardless of who wins: **MOVE THE QUESTION FROM PITCH TYPE TO PITCH LOCATION.** A pitcher who throws three pitch types but puts all of them in the same third of the zone is more predictable than his mix suggests, and no pitch-type analysis will ever see it. **Add a location-by-count row to the count audit** (`library/pitchability-sequencing.md` §6). DO NOT CITE PAPER 1 BY AUTHOR — this entry cannot supply one.
CONFIDENCE: low-medium — direction corroborated by two sources, magnitudes absent, one citation incomplete
SEE ALSO: F-254, F-255, F-262, F-259

### F-262 | The seven-class predictability baseline — an average MLB pitcher's modal pitch is thrown 54.38% of the time
TOPIC: pitchability, predictability, pitch prediction, machine learning, naive accuracy, baseline, count, previous pitch, measurement
CLAIM: Across 287 MLB pitchers and ~1.34 million pitches, always guessing a pitcher's most-thrown pitch is correct 54.38% of the time — a descriptive baseline for auditing an individual athlete, not an estimate of what a hitter knows.
NUMBERS: ~**1,340,000** pitch records from **287 pitchers** who threw at least 500 pitches in **BOTH the 2014 and 2015** MLB seasons. **Seven-class** problem: fastball, cutter, sinker, slider, curveball, changeup, knuckleball. **NAIVE BASELINE ACCURACY 54.38%.** Methods compared: multiclass LDA, multiclass SVM, random forest. **Random forests outperformed LDA and SVM "by a wide margin"**; LDA second, SVM third; LDA reported more computationally efficient and more consistent. **NO absolute model accuracy above baseline was retrievable — only the ordering.** Most useful predictive features reported: **pitcher/batter history, pitcher/count history, THE PREVIOUS PITCH, and the game score.**
POPULATION: MLB
EVIDENCE: EMERGING — SNIPPET-ONLY, NOT SOURCE-VERIFIED (peer-reviewed sports-analytics journal, very large sample; the headline gain over baseline could not be retrieved)
CAUSALITY: CROSS_SECTIONAL
SOURCE: Sidle G, Tran H (2018), "Using multi-class classification methods to predict baseball pitch types," Journal of Sports Analytics, DOI 10.3233/JSA-170171. NOT OPENED — doi.org, journals.sagepub.com and researchgate.net all EGRESS_BLOCKED 2026-09-03.
COACHING: THIS IS THE SEVEN-CLASS COMPANION TO F-255's BINARY AND 3-CLASS FIGURES, AND THE THREE ARE NOT POOLABLE — different label sets, different papers, different years. Quote each with its class count or the numbers become nonsense. WHAT THE 54.38% IS: a description of PITCH MIX, and a baseline any coach can compute on his own athlete in an afternoon. WHAT IT IS NOT: an estimate of how often a hitter knows what is coming. A hitter is not a random forest with a full database; he has ~150 ms. **NEVER SAY "PITCHERS ARE 54% PREDICTABLE."** CORROBORATION WORTH NOTING: "the previous pitch" again lands among the top features, exactly as in F-255 — the pitcher's own past behaviour predicts his next choice. **That is a fact about the PITCHER, not about the hitter, and it is precisely why the count audit (usage) is the deployable action and the order argument is not.** SAMPLE SIZE FOR THE AUDIT, complementing the +/-10pp figure already in `pitchability-sequencing.md` §6: to DETECT a shift in one count's fastball usage from 70% to 55% at 80% power, alpha = .05, requires **APPROXIMATELY 83 PITCHES IN THAT COUNT**. And to detect a lag-1 serial correlation of r = 0.10 at 80% power requires **APPROXIMATELY 790 PITCHES** — roughly a starter's full season. Do not run either on a month.
CONFIDENCE: medium-high on the 54.38% baseline (a simple descriptive statistic on a very large sample); low on anything about exploitability
SEE ALSO: F-255, F-261, F-257

### F-263 | The TTOP framing this corpus should adopt — "third time through" is a BIN BOUNDARY, and the command test would settle the mechanism
TOPIC: times through the order, TTOP, binning artifact, fatigue, familiarity, command, infraspinatus, study design, cheapest study
CLAIM: The times-through-the-order penalty is the stride-length error in the time domain — a smooth process chopped into three bins produces bin-to-bin differences that get narrated as discrete events — and the corpus's own fatigue physiology supplies a cheap, unrun test of the mechanism.
NUMBERS: No new numbers. Extends F-256. The structural argument: every pre-2023 TTOP analysis **binned plate appearances by time-through and compared bin means**, and that design **cannot distinguish a step from a slope**, because a slope binned into three buckets produces three different bucket means. Brill/Deshpande/Wyner's stated critique is exactly this — prior work "cannot disentangle continuous evolution in pitcher performance over the course of a game from discontinuities between successive TTOs." THE UNRUN TEST: F-125 and F-127 hold that under fatigue **command degrades BEFORE velocity** and that fatigue **neurally inhibits the infraspinatus**. Therefore a fatigue-driven TTOP should appear in **LOCATION QUALITY, smoothly, AHEAD of any outcome signal.** A familiarity-driven one should not. Computable from public pitch-level data with no new collection.
POPULATION: MLB
EVIDENCE: ESTABLISHED as a statement about study design; the mechanism claim is a testable prediction, not a result
CAUSALITY: MECHANISM
SOURCE: `daily/2026-09-03-report.md`; extends F-256; predicts from F-125 and F-127.
COACHING: THE SENTENCE FOR THE ATHLETE: **"Nothing happens to you when the lineup flips. There's no bell. You get tired on a pitch-count clock, and the clock doesn't care what inning it is."** THE SENTENCE FOR THE STAFF: "the third time through the order" is a **bin boundary** — it is not obviously a thing that happens to a pitcher. This corpus has already been burned by exactly this shape once, in space rather than time: stride length was a real cross-sectional pattern read as a mechanism that would survive intervention (F-043), and F-094 is the general warning. WHAT BOTH CAMPS AGREE ON, so do not overcorrect: **the batter does get better across a game.** The dispute is about WHY, and therefore about what to do — **fatigue is trainable and familiarity is not.** THE STUDY: track a location-quality metric against pitch count within starts and race it against outcome-based TTOP. **This is the cheapest high-value study the 2026-09-03 cycle identified and nobody appears to have run it.** ONE THING BLOCKS PRICING THE FATIGUE SIDE AT ALL: the within-start velocity decline in mph is unknown to this corpus (F-266).
CONFIDENCE: high on the design argument; the mechanism remains open (Dispute #18)
SEE ALSO: F-256, F-043, F-094, F-125, F-127, F-266

### F-264 | The ABS challenge system is live in MLB for 2026 — and the catcher, not the pitcher, should own the challenge
TOPIC: ABS, automated ball-strike, challenge system, framing, catcher, command measurement, rules, 2026, overturn rate
CLAIM: MLB's automated ball-strike CHALLENGE system took effect for the 2026 regular season, and the overturn rates by challenger say the catcher is markedly the best-calibrated of the three eligible parties and the pitcher markedly the worst.
NUMBERS: MECHANICS: **two challenges per team over the first nine innings**, retained on a successful challenge; only the **pitcher, catcher or batter** may initiate, by **tapping the head immediately** after the umpire's call. OVERTURN RATES: **Triple-A 2025 = 50%; spring training = 52%.** BY CHALLENGER: **catchers 56%, batters 50%, PITCHERS 41%.** PLAYER SENTIMENT: an Athletic poll of **134 players** found **63.4% opposed, 17.2% in favor.** Public trackers: Baseball-Reference `/friv/abs-challenges.shtml`, ESPN 2026 ABS tracker. **This is the CHALLENGE system, NOT full ABS** — the umpire still calls every unchallenged pitch.
POPULATION: MLB (2026); Triple-A (2025 overturn rate)
EVIDENCE: ESTABLISHED as a rules fact — SNIPPET-ONLY on every percentage (widely reported across independent outlets; no primary document opened)
CAUSALITY: not applicable (a rule change)
SOURCE: MLB.com, ESPN 2026 ABS challenge tracker, Baseball-Reference ABS challenge analysis page, The Athletic player poll (n = 134). All reported 2026; none opened — EGRESS_BLOCKED 2026-09-03.
COACHING: ⚠️ **A SEPARATE ABS ENTRY EXISTS ON THE UNMERGED `daily/2026-09-01-hitter-perception` BRANCH (its F-257), which additionally flags a widely circulating ABS effect figure as not credible. Read both; they are not duplicates — that one is about a bad number, this one is about the challenger splits. See F-259.** THE ONE ACTIONABLE ITEM, and it follows from the splits alone: **IF YOUR CONFERENCE ADOPTS A CHALLENGE SYSTEM, THE CATCHER OWNS THE CHALLENGE.** Pitchers are the worst challengers at 41% — worst view of the plate of the three eligible parties, and most emotionally invested in the call. WHY A PITCHING-DEVELOPMENT VAULT CARES BEYOND THE TACTIC: F-182 registers that **framing contaminates command measurement** — this corpus cannot separate a pitcher's command from his catcher's receiving, and that confound sits underneath Dispute #12. ABS-challenge data is the first public, umpire-independent ground truth on borderline pitches at the top level of the sport. **LIMIT THAT MUST TRAVEL WITH IT: challenged pitches are a SELECTED subset** — challenges are spent strategically — **so this is not a random sample of borderline calls.** TWO SPECULATIONS, LABELLED AND NOT TO BE COACHED: if framing value shrinks, the marginal value of a pitcher who lives off the plate edge falls and the value of one who commands inside the true zone rises. Nobody has measured it; 2026 is the first season.
CONFIDENCE: high that the system exists and how it works; medium on the specific percentages; none on any downstream effect on pitcher value
SEE ALSO: F-182, F-169, F-170

### F-265 | The "vertical slider" — PROMISING physics, UNPROVEN acquisition cost
TOPIC: pitch design, vertical slider, gyro, arsenal construction, velocity tax, Driveline, adoption, field sweep, watch item
CLAIM: Driveline is arguing that a hard, gyro-heavy slider with vertical rather than horizontal break is the next pitch to scale across baseball, on the grounds that it carries a smaller velocity tax than a conventional breaking ball.
NUMBERS: A hard, gyro-heavy slider with little horizontal movement and meaningful drop — **6 to 9 INCHES of additional drop at AROUND 84 MPH**. Classification threshold given as **WITHIN 10 MPH of the pitcher's own fastball**. CURRENT USAGE: **"around one percent of pitches"** fall into the hard-with-drop window, described by Driveline director of pitching **Connor White** as the highest rate they have seen and "early in the adoption phase." **NO run values, NO whiff rates, NO sample size, NO acquisition data of any kind published.**
POPULATION: MLB (usage figure)
EVIDENCE: WEAK — SNIPPET-ONLY (industry grey literature from a commercial training provider; no outcome data; drivelinebaseball.com is on this program's WebFetch blocklist and was additionally EGRESS_BLOCKED 2026-09-03)
CAUSALITY: CROSS_SECTIONAL (usage description only)
SOURCE: Driveline Baseball, "Is the vertical slider the next big thing in pitch design?", drivelinebaseball.com/blogs/blog/vertical-slider-pitch-design. NOT OPENED.
COACHING: THE VELOCITY-TAX FRAMING IS A GENUINE ARSENAL-CONSTRUCTION ARGUMENT, NOT A MARKETING LINE — a breaking ball inside 10 mph of the fastball is a different tunnelling and timing proposition from one 15 mph off it, and that is consistent with what this corpus holds on velocity separation (F-159, F-232) and with the 09-02 sweep's finding that the industry is converging on shape diversification (its Sweep 3 #3). **THE CLAIM TO REFUSE: that it "isn't especially difficult to acquire."** Nothing published supports it, and it is exactly the class of claim that has been wrong before — spin efficiency was sold as trainable and is ~65% fixed over three years, with the one documented mover having bought a delivery change (F-235, F-153). **VERDICT: PROMISING on the physics, UNPROVEN on acquisition cost. Do not build an off-season around it.** Watch item.
CONFIDENCE: medium that the pitch class is real and rising; none on acquisition cost or outcome value
SEE ALSO: F-235, F-153, F-159, F-232

### F-266 | FOLKLORE — the "3 to 5 mph within-start velocity decline," and the corpus has NO replacement number
TOPIC: velocity, fatigue, within-game decline, inning, folklore, unsourced, TTOP, named gap
CLAIM: The claim that a starter loses 3-5 mph between the first and sixth innings traces to a commercial training blog, is stated about an amateur-level pitcher, could not be corroborated from any analytical source, and this corpus cannot supply a correct figure to replace it.
NUMBERS: THE CLAIM AS CIRCULATED: "a pitcher sitting 88 in the first inning often dips 3-5 mph by the sixth as he tires." **Note the baseline given is 88 MPH — an amateur illustration, not an MLB measurement.** THREE DIFFERENTLY-WORDED SEARCHES FAILED TO CORROBORATE IT from any analytical or peer-reviewed source. **THE CORPUS HAS NO REPLACEMENT NUMBER.** It is inconsistent with F-082 (in-season velocity RISES ~0.6 mph rather than decaying). CONTEXT FIGURES OBTAINED IN THE SAME SWEEP, themselves snippet-only: MLB four-seam fastballs averaged **94.7 mph in 2026**, up from 94.5 the prior year and 91.9 in 2008; starters average **just under 5 1/3 innings on 86.2 pitches**, down from 6 1/3 innings in the 1980s.
POPULATION: the claim is stated about an 88 mph pitcher — off_population, NOT an MLB figure
EVIDENCE: FOLKLORE (commercial training blog, amateur illustration, uncorroborated across three searches)
CAUSALITY: not applicable
SOURCE: circulating on commercial pitching-training blogs; surfaced in the 2026-09-03 sweep. No analytical or peer-reviewed source located.
COACHING: **DO NOT QUOTE 3-5 MPH. AND DO NOT QUOTE A COUNTER-NUMBER EITHER** — this cycle could not obtain one, and inventing a figure to displace a bad figure is the same error in the other direction. NOTE THE FAMILY RESEMBLANCE: this corpus already catches facilities quoting "3-5 mph" as a *training gain* to an 88 mph arm, extrapolated from 15-year-olds (F-072). **The same number is now circulating as a within-start LOSS, on the same 88 mph strawman.** WHAT TO SAY: "That comes from a training blog about an 88 mph pitcher and nobody has shown it holds for a professional starter." WHY IT MATTERS BEYOND PEDANTRY: the within-start decline is the **load-bearing quantity in the fatigue-versus-familiarity explanation of the TTOP** (F-263, Dispute #18). If the decline is large, fatigue is the mechanism; if small, familiarity or nothing is. **THE CORPUS HAS NO NUMBER FOR A QUANTITY EVERY COACH BELIEVES HE KNOWS.** Registered as a named gap and a priority for the next cycle.
CONFIDENCE: high that the circulating claim is unsourced and mislevelled; ZERO knowledge of the true magnitude
SEE ALSO: F-082, F-072, F-263, F-125

---

*End of registry. 266 entries on this branch. Numbering is stable — never reuse or renumber an F-ID. **⚠️ F-252 to F-258 are AMBIGUOUS across the unmerged `daily/2026-09-01-hitter-perception` branch — always cite them with their branch until the merge is resolved (F-259).** New findings append from F-267.*
